...
Mesh::Mesh(const std::string& name, const aiMesh *mesh_ai) : Mesh(name) {
	...
	tangents.reserve(mesh_ai->HasTangentsAndBitangents() ? mesh_ai->mNumVertices : 0);
    for (uint32_t i = 0; i < mesh_ai->mNumVertices; ++i) {
		...
		if (mesh_ai->HasTangentsAndBitangents())
			tangents.emplace_back(to_glm(mesh_ai->mTangents[i]));
    }
	...
	if (not tangents.empty())
		add_vertex_buffer(GL_FLOAT, 3, tangents.size(), tangents.data());
