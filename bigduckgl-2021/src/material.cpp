...
Material::Material(const std::string& name, const std::string& base_path, const aiMaterial* mat_ai) : Material(name) {
	...
	if (mat_ai->Get(AI_MATKEY_COLOR_DIFFUSE, diff) == AI_SUCCESS) {
		k_diff = glm::vec4(diff.r, diff.g, diff.b, 1.0f);
    }
#endif
...
#if PLAIN_COLOR_SUPPORT == true
    if (mat_ai->Get(AI_MATKEY_COLOR_SPECULAR, spec) == AI_SUCCESS) {
		k_spec = glm::vec4(spec.r, spec.g, spec.b, 1.0f);
    }
#endif
	if (mat_ai->Get(AI_MATKEY_COLOR_AMBIENT, diff) == AI_SUCCESS) {
		k_amb = glm::vec4(diff.r, diff.g, diff.b, 1.0f);
	}
    if (mat_ai->GetTextureCount(aiTextureType_HEIGHT) > 0) {
        aiString path_ai;
        mat_ai->GetTexture(aiTextureType_HEIGHT, 0, &path_ai);
        add_texture("normalmap", make_texture(name + "_" + get_filename(path_ai.C_Str()), path_ai.C_Str()));
    }
    if (mat_ai->GetTextureCount(aiTextureType_OPACITY) > 0) {
        aiString path_ai;
        mat_ai->GetTexture(aiTextureType_OPACITY, 0, &path_ai);
        add_texture("alphamap", make_texture(name + "_" + get_filename(path_ai.C_Str()), path_ai.C_Str()));
    }
...
#if PLAIN_COLOR_SUPPORT == true
	shader->uniform("k_amb", k_amb);
	shader->uniform("k_diff", k_diff);
	shader->uniform("k_spec", k_spec);
#endif
}
