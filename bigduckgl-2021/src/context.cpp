...
static void display_camera(Camera* cam) {
    ImGui::Indent();
	ImGui::Text("addr %lu", (unsigned long)cam);
    ...
    if (ImGui::Button("Print to terminal")) {
#define pr(X) std::cout << cam->name << "->" #X << " = glm::vec3(" << cam->X.x << ", " << cam->X.y << ", " << cam->X.z << ");"  << std::endl
		pr(pos);
		pr(dir);
		pr(up);
	}
...
static void parse_gl_header() {
    std::stringstream input(read_file(concat(GLEW_INCLUDE_PATH, "GL/glew.h")) + read_file(concat(OPENGL_INCLUDE_PATH, "GL/gl.h")));
    std::string line;
